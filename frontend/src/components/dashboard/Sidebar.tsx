...
export default function Sidebar({ business, mobile }: { business?: any; mobile?: boolean }) {
  ...
  if (mobile) {
    return (
      <aside
        className="fixed inset-y-0 left-0 z-50 w-[88vw] max-w-72 flex-shrink-0 flex flex-col"
    ...
    >
            ...
            >
              ...
              {label === 'AI Assistant' && (
                <span
                  ...
                  style={{ background: 'rgba(247,168,37,0.2)', color: '#F7A825' }}
                >
                  AI
                </span>
              )}
      ...
      <div className="px-4 pb-2">
        ...
        >
          ...
          {unreadCount > 0 && (
            <span
              className="ml-auto text-[10px] px-1.5 py-0.5 rounded-full font-bold min-w-[18px] text-center"
              style={{ background: '#F7A825', color: '#0A0B0F' }}
            >
              {unreadCount > 99 ? '99+' : unreadCount}
            </span>
