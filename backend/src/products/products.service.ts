...
export class ProductsService {
  ...
  async findAll(businessId: string, filters: { category?: string; lowStock?: boolean } = {}) {
  ...
  async create(businessId: string, data: { name: string; category?: string; quantity: number; price: number }) {
    return this.prisma.product.create({ data: { businessId, ...data } });
  }

  async update(id: string, data: Partial<{ name: string; category: string; quantity: number; price: number }>) {
